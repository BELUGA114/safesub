...
3. 返回 SafeSub 页面，填入第三方订阅 URL，选择最终订阅的传输类型，生成最终订阅链接
4. 在客户端中添加最终订阅链接。客户端的每次更新都会即时拉取第三方订阅并恢复真实数据

伪装节点的传输类型只影响诱饵参数的外观，不影响最终订阅。第三方订阅中的伪装节点无论以 ws 还是 xhttp 传输返回都能被恢复。

最终订阅的传输选择缺省为“跟随真实节点”，即保留第一步输入的真实节点的传输与参数；选择 WebSocket 或 XHTTP 会把恢复后节点的 `type` 重写为所选传输（XHTTP 时补充缺省 `mode=auto`），其余参数不变。此选项适用于同一端点同时提供两种传输的情况；若两种传输在服务器上使用不同路径，请直接在第一步填入对应传输的真实节点。
